...
load('SPX_20071123', 'tday', 'stocks', 'op');
...
dailyret=(op-lag1(op))./lag1(op); % daily returns
...
weights(~isfinite(op) | ~isfinite(lag1(op)))=0; % those stocks that do not have valid prices or daily returns are excluded.
